...
class LocationDatabase {
  ...
  static Future<LocationDatabase> open() async {
    final Database db =
        await openDatabase(dbName, version: 1, onCreate: (Database db, int version) async {
  ...
  Future<List<location.Location>> getLocations() async {
    ...
    return locationEntries
        .map((entry) => location.Location.fromJson(Map<String, dynamic>.from(entry)))
        .toList();
  }
  ...
  void insertLocation(location.Location location) async {
    ...
    _db.transaction((txn) async {
      await txn.rawInsert(query,
          [location.city, location.state, location.zip, location.latitude, location.longitude]);
    });
  }
